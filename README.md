...
app.post('/request_medicine/',(req,res,next) => {
    ...
    var sql = 'INSERT INTO administratoin_request_form (babyname, request_day,symptom, medicine, cc, numberoftimes, medicine_time, storage, baby_comment, kindergarten, classname, parents_id, baby_image, write_time) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
    con.query(sql,[babyname,request_day,symptom,medicine,cc,numberoftimes,medicine_time,storage,baby_comment,kindergarten,classname, parents_id, baby_image, write_time], function (error, result, fields) {
        if (error) {
            console.log("error ocurred", error);
            res.send({
                "code": 400,
