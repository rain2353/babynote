...
class consent_text_Adapter(
    internal val context: Context,
    internal val consent_comment_list: List<consent_comment_list>
) :
    RecyclerView.Adapter<consent_text_Adapter.Holder>()    {
    lateinit var myAPI: INodeJS
    var compositeDisposable = CompositeDisposable()
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): consent_text_Adapter.Holder {
        val view = LayoutInflater.from(context).inflate(R.layout.notice_comment_item, p0, false)
        return Holder(view)
    }

    override fun getItemCount(): Int {
        return consent_comment_list.size
    }

    override fun onBindViewHolder(p0: consent_text_Adapter.Holder, p1: Int) {
        p0.comment_nickname.text = consent_comment_list[p1].comment_nickname
        p0.comment_time.text = consent_comment_list[p1].comment_time
        p0.comment_content.text = consent_comment_list[p1].comment_content
        ...
        p0.setClick(object : IRecyclerOnClick {
            ...
            override fun onLongClick(view: View, Position: Int) {
                Common.selected_consent_comment = consent_comment_list[Position]
                //Init API
                val retrofit = RetrofitClient.instance
                myAPI = retrofit.create(INodeJS::class.java)

                if (consent_comment_list[p1].comment_writer == Common.selected_baby?.parents_id) {
                    val intent = Intent(view.context, consent_comment_modify::class.java)
                    intent.putExtra("댓글",consent_comment_list[p1].comment_content)
