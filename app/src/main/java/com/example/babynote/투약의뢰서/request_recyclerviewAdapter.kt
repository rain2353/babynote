...
class request_recyclerviewAdapter(internal val context: Context, internal val request_list: List<administration_request_list>) :
    RecyclerView.Adapter<request_recyclerviewAdapter.Holder>() {
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): request_recyclerviewAdapter.Holder {
    ...
    override fun onBindViewHolder(p0: request_recyclerviewAdapter.Holder, p1: Int) {
        Picasso.get().load(request_list[p1].baby_image).resize(250,250).into(p0.item_babyimage)
        p0.item_day.text = request_list[p1].request_day
        p0.item_babyname.text = request_list[p1].babyname
        p0.item_classname.text = request_list[p1].classname
        ...
        p0.setClick(object : IRecyclerOnClick {
            ...
            override fun onClick(view: View, position: Int) {
                Common.selected_administration_request_form = request_list[position]
                val intent = Intent(view.context, request_text::class.java)
